...
class TestEnKS:
    """Test suite for EnKS data assimilation."""

    @pytest.fixture
    def basic_setup(self):
        """Basic setup for EnKS tests."""
        n_members = 10
        lag = 5
        n_variables = 5
        n_time_steps = 3
        ...
        # Create EnKS object
        enks_obj = EnKS(
            n_members=n_members, n_variables=n_variables, lag=lag, initial_ensemble_std=initial_ensemble_std
        )
    ...
    def test_enks_initialization(self, basic_setup):
        """Test EnKS initialization."""
        setup = basic_setup
        enks_obj = setup["enks_obj"]

        assert enks_obj.n_members == setup["n_members"]
        assert enks_obj.n_variables == setup["n_variables"]
        assert enks_obj.lag == setup["lag"]
        assert enks_obj.initial_ensemble_std == setup["initial_ensemble_std"]
        assert enks_obj.ensemble is None  # Not initialized yet
    ...
    def test_ensemble_initialization(self, basic_setup):
        ...
        # Initialize ensemble
        ensemble_state = enks_obj.initialize_ensemble(x_init)

        # Check shapes
        assert ensemble_state.fields["x"].shape == (setup["n_members"], 1, setup["n_variables"])
        assert enks_obj.ensemble is not None
    ...
    def test_linearize_observation_operator(self, basic_setup, mock_observations):
        ...
        for t in range(basic_setup["n_time_steps"]):
            H_t = obs_op.linearize(idx_time=t)
    ...
    def test_forecast_step(self, basic_setup, linear_dynamics):
        ...
        # Initialize ensemble
        enks_obj.initialize_ensemble(x_init)
        initial_ensemble = enks_obj.ensemble.fields["x"].clone()

        # Perform forecast step
        enks_obj.forecast_step(linear_dynamics, setup["dt"], 1, None, None)

        # Check that ensemble has evolved
        final_ensemble = enks_obj.ensemble.fields["x"]
        ...
        if enks_obj.ensemble.time_axis is not None:
            assert torch.isclose(enks_obj.ensemble.time_axis[-1], torch.tensor(expected_time))
    ...
    def test_analysis_step(self, basic_setup, mock_observations):
        ...
        H_t = obs_op.linearize(idx_time=0)
        ...
        if H_t.shape[0] > 0:  # Only test if there are observations
            ...
            pre_analysis = enks_obj.ensemble.fields["x"][:, -1, :].clone()

            # Perform analysis step
            mask_t0 = observations.mask.fields["x"][0, 0, :]
            observations_t0 = observations.state.fields["x"][0, 0, mask_t0]
            enks_obj.analysis_step(H_t, observations_t0, obs_op, R_t, 0)

            # Check that ensemble has been updated
            post_analysis = enks_obj.ensemble.fields["x"][:, -1, :]
    ...
    def test_assimilate_method(self, basic_setup, mock_observations, linear_dynamics):
        ...
        enks_result = enks_obj.assimilate(
        ...
        assert enks_result.fields["x"].shape == (setup["n_members"], n_steps, setup["n_variables"])

    def test_enks_function(self, basic_setup, mock_observations, linear_dynamics):
        """Test the standalone enks_assimilate function."""
        setup = basic_setup
        x_init = setup["x_init"]
        ...
        for linearize in [False, True]:
            ...
            # Check results
            assert enks_result.fields["x"].shape == (n_members, n_steps, setup["n_variables"])
    ...
    def test_get_ensemble_statistics(self, basic_setup):
        ...
        # Get statistics
        stats = enks_obj.get_ensemble_statistics()
    ...
    def test_error_handling(self, basic_setup):
        ...
        with pytest.raises(ValueError, match="x_init must be provided to initialize the ensemble"):
            enks_obj.assimilate(m_dyn=lambda x, dt, di, si: x, observations=None, obs_op=None, x_init=None)
    ...
    def test_ensemble_convergence(self, basic_setup, mock_observations, linear_dynamics):
        ...
        initial_spread = enks_result.fields["x"].std(dim=0).mean()
        final_spread = enks_result.fields["x"].std(dim=0).mean()
...
class TestEnKSIntegration:
    """Integration tests for EnKS with realistic scenarios."""
    ...
    def test_lorenz96_like_system(self, mock_observations_integration):
        """Test EnKS with a Lorenz-96 like system using realistic observations."""
        n_variables = 10
        n_members = 20
        lag = 10
        n_steps = 10
        ...
        for linearize in [True, False]:
            ...
            # Basic checks
            assert len(enks_result.fields["x"].shape) == 3
            assert len(enks_result.fields["x"].mean(dim=0)) == n_steps

            # Check that system evolved
            initial_mean = enks_result.fields["x"][0].mean(dim=0)
            final_mean = enks_result.fields["x"][-1].mean(dim=0)
            assert not torch.allclose(initial_mean, final_mean, atol=1e-3)

            # Check ensemble spread is reasonable
            final_spread = enks_result.fields["x"][-1].std(dim=0).mean()
            ...
            assert len(enks_result.fields["x"].mean(dim=0)) == n_steps, "Should have evolved over multiple time steps"
